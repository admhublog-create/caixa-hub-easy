...
type I={created_at:string;total_rolos:number};
export function TapePage(){
 const[rr,setR]=useState<R[]>([]),[ee,setE]=useState<E[]>([]),[inv,setInv]=useState<I|null>(null),[purchases,setPurchases]=useState<P[]>([]);
 const[boxes,setBoxes]=useState("0"),[rolls,setRolls]=useState("0"),[kg,setKg]=useState(""),[value,setValue]=useState(""),[supplier,setSupplier]=useState(""),[note,setNote]=useState(""),[msg,setMsg]=useState("");
 async function load(){if(!supabase)return;const[r,e,i,p]=await Promise.all([
 ...
 ]);setR((r.data||[]) as R[]);setE((e.data||[]) as E[]);setInv((i.data?.[0]||null) as I|null);setPurchases((p.data||[]) as P[])}
 useEffect(()=>{load()},[]);
 ...
 return <><div className="mb-7"><p className="mb-1 text-xs font-bold uppercase tracking-[.16em] text-[var(--primary)]">Controle operacional</p><h2 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Fita Gomada</h2><p className="mt-1 text-sm text-[var(--muted)]">Controle de estoque físico. 1 caixa = 15 rolos.</p></div>
 <section className="grid gap-4 md:grid-cols-3"><KpiCard title="Estoque atual" value={stock} detail="rolos disponíveis" Icon={CircleDot}/><KpiCard title="Entradas" value={ee.length} detail="movimentações físicas" Icon={PackageOpen}/><KpiCard title="Retiradas" value={rr.length} detail="movimentações registradas" Icon={History}/></section>
 <div className="mt-6 grid gap-6"><TapeWithdrawal onSaved={load}/><Card><CardContent><h3 className="mb-1 font-bold">Nova entrada de fita gomada</h3><p className="mb-4 text-sm text-[var(--muted)]">Esta operação altera o estoque físico.</p>
 ...
 <TapeInventory stock={stock} onSaved={load}/><TapePurchases onSaved={load}/><TapeHistory withdrawals={rr} entries={ee} purchases={purchases}/></div></>}
