...
export function TapePage(){
 ...
 const[boxes,setBoxes]=useState("0"),[rolls,setRolls]=useState("0"),[kg,setKg]=useState(""),[value,setValue]=useState(""),[supplier,setSupplier]=useState(""),[note,setNote]=useState(""),[msg,setMsg]=useState(""),[adding,setAdding]=useState(false),[loadError,setLoadError]=useState(""),[deletingPurchase,setDeletingPurchase]=useState<string|null>(null);
 ...
 useEffect(()=>{load()},[]);async function deletePurchase(p:P){if(!supabase||loadError)return;if(!window.confirm(`Excluir esta compra histórica de ${p.competencia}? Este registro é financeiro e não altera o estoque físico.`))return;setDeletingPurchase(p.id);const{error}=await supabase.from("fita_compras_historicas").delete().eq("id",p.id);setDeletingPurchase(null);if(error){setMsg("Não foi possível excluir a compra histórica.");return}setMsg("Compra histórica excluída.");await load()}
 ...
 async function add(){if(loadError){setMsg("Entrada indisponível enquanto os dados da fita não forem carregados.");return}const cx=Math.max(0,Number(boxes)||0),av=Math.max(0,Number(rolls)||0),total=cx*PER_BOX+av;if(!supabase||!total){setMsg("Informe caixas ou rolos para registrar a entrada.");return}setAdding(true);setMsg("");const{error}=await supabase.from("fita_entradas").insert({caixas:cx,rolos_avulsos:av,total_rolos:total,kg:Number(kg)||null,valor:Number(value)||null,fornecedor:supplier.trim()||null,observacao:note.trim()||null});setAdding(false);if(error){setMsg("Não foi possível registrar a entrada.");return}setBoxes("0");setRolls("0");setKg("");setValue("");setSupplier("");setNote("");setMsg("Entrada registrada com sucesso.");await load()}
 ...
 <TapeInventory stock={stock} onSaved={load}/><TapePurchases onSaved={load}/><TapeHistory withdrawals={rr} entries={ee} purchases={purchases} inventories={inventories} onDeletePurchase={deletePurchase} deletingPurchase={deletingPurchase}/></div>}</>}
